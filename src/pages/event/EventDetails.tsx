      ...
      if (eid && eid !== 'default') {
        ...
        if (eventData) {
          ...
          // Priority: Event Image > Profile Image > Global/Default
           ...
           let resolvedHeroImg = settingsData?.find(s => s.key === 'event_hero_image')?.value || eventHero;
           ...
           if (eventData.event_profile_id) {
             ...
             if (profileData) {
               if (profileData.event_hero_image) {
                 resolvedHeroImg = profileData.event_hero_image;
               }
           ...
           if (eventData.event_image_url) {
              resolvedHeroImg = eventData.event_image_url;
           }
          ...
          // Check if specific location exists in settings for this event (using UUID)
          const { data: locData } = await supabase.from('site_settings').select('value').eq('key', `event_location_${eventData.id}`).maybeSingle();
  ...
  const timingInfo = useMemo(() => {
    ...
    if (now < start) {
      ...
      return {
        ...
        secondary: format(start, "MMM dd, yyyy • HH:mm")
      };
    }
        ...
        <div className="px-4 py-6 animate-slide-up">
          <div className="flex items-center justify-between gap-4 mb-4">
            ...
            <Dialog>
              ...
              <DialogContent className="max-w-[90vw] sm:max-w-lg rounded-[2.5rem] p-0 overflow-hidden border-none shadow-2xl bg-white/95 backdrop-blur-xl animate-in fade-in zoom-in-95 duration-300">
          ...
          {timingInfo && (
            <div className="mb-6 rounded-3xl border border-slate-200 bg-white/90 backdrop-blur-sm shadow-sm p-5 flex items-center justify-between gap-4">
              <div>
                <p className="text-[11px] font-black uppercase tracking-widest text-slate-400">Time Remaining</p>
                <p className="text-lg font-black text-slate-900 mt-2">{timingInfo.primary}</p>
                {timingInfo.secondary && (
                  <p className="text-[11px] font-black uppercase tracking-widest text-slate-500 mt-1">{timingInfo.secondary}</p>
                )}
              </div>
              <div className="w-14 h-14 rounded-2xl bg-event-pink/10 text-event-pink flex items-center justify-center overflow-hidden border border-event-pink/10">
                <img src={heroImg} className="w-full h-full object-cover" alt="" />
              </div>
            </div>
          )}

          <div className="mb-6 rounded-3xl border border-slate-200 bg-white/90 backdrop-blur-sm shadow-sm p-5 flex flex-col sm:flex-row gap-5 items-start sm:items-center justify-between">
            <div>
              <p className="text-[11px] font-black uppercase tracking-widest text-slate-400">Event QR</p>
              <h3 className="text-lg font-black text-slate-900 mt-2">Show this at the entrance</h3>
              <p className="text-[11px] font-black uppercase tracking-widest text-slate-500 mt-2">Scan to open registration</p>
            </div>
            <div className="w-full sm:w-auto flex items-center justify-center">
              <div className="bg-white p-3 rounded-2xl border border-slate-200">
                <QRCode value={qrValue} size={128} className="h-32 w-32" />
              </div>
            </div>
          </div>
                ...
                {programItems.map((item, index) => (
                  <div key={`${item.title}-${index}`} className="flex gap-4 group">
                    <div className="w-14 shrink-0 flex flex-col pt-1">
                      <span className="text-[13px] font-bold text-event-pink">{item.time || "—"}</span>
                      {item.location && (
                        <span className="text-[10px] font-bold text-slate-400 uppercase">{item.location}</span>
                      )}
                    </div>
                    <div className="flex-1 p-4 rounded-2xl border border-slate-200 bg-white shadow border-l-4 border-l-event-pink group-hover:shadow-md transition-shadow">
          ...
          {/* Schedule */}
          <div className="mb-4">
            <h3 className="font-bold text-lg mb-5 text-slate-900">Schedule</h3>
            <div className="space-y-4">
              {config.schedule.map((item, index) => (
                ...
                >
                  <div className="w-14 shrink-0 flex flex-col pt-1">
                    <span className="text-[13px] font-bold text-event-pink">{item.time.split(' ')[0]}</span>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-xl border-t border-slate-200 z-50">
        <div className="max-w-lg mx-auto flex gap-3">
          <Button
            variant="secondary"
            size="icon"
            className={`shrink-0 w-14 h-14 rounded-2xl transition-all shadow-sm ${isBookmarked ? "text-event-pink border-event-pink bg-event-pink/5" : "text-slate-400 border-slate-200"}`}
          ...
          <Button 
            onClick={() => navigate(`/event/register?${searchParams.toString()}`)}
            className="flex-1 gradient-event text-white font-bold h-14 rounded-2xl shadow-lg shadow-event-pink/20 hover:opacity-90 transition-all active:scale-[0.98]"
          >
            Register Now
